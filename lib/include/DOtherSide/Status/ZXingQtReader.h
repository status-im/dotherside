...
inline Result ReadBarcode(const QVideoFrame& frame, const DecodeHints& hints = {})
{
	...
	if (fmt != ImageFormat::None) {
		...
		QImage::Format imageFormat = QVideoFrame::imageFormatFromPixelFormat(img.pixelFormat());
		if (imageFormat != QImage::Format_Invalid) {
			QImage image = QImage(frame.bits(), frame.width(), frame.height(), frame.bytesPerLine(), imageFormat);
			res = ReadBarcode(image, hints);
		}
	}
